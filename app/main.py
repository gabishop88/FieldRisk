...
def run_apsim() -> None:
    result = subprocess.run(
        ["Rscript", "/simulations/simulation_script.R"], capture_output=True
    )
    print(result.stdout)
    if result.stderr:
...
def load_data():
    ts = pd.read_csv(
        os.path.join(BASE, "weather_data/season_ndvi_timeseries.csv"),
        parse_dates=["date"],
    )
    stress = pd.read_csv(
        os.path.join(BASE, "weather_data/season_ndvi_stress_events.csv"),
        parse_dates=["date"],
    )
    pheno = pd.read_csv(os.path.join(BASE, "weather_data/season_ndvi_phenology.csv"))
    forecast = pd.read_csv(
        os.path.join(BASE, "weather_data/forecast_all.csv"), parse_dates=["date"]
    )
...
if is_sce:
    st.info(
        "NDVI/weather sections are not available for the simulated site. See Crop Simulation Results below."
    )
...
if is_sce:
    ...
    harv_trials = sce_trials[
        sce_trials["Result"].str.contains("Harvested", case=False, na=False)
    ]
    fail_trials = sce_trials[
        ~sce_trials["Result"].str.contains("Harvested", case=False, na=False)
    ]
    if not harv_trials.empty:
        best = harv_trials.loc[harv_trials["Yield_Sim"].idxmax()]
        st.success(
            f"**Recommended maturity group: {best['Mat']}** — highest simulated yield at {best['Yield_Sim']:.0f} kg/ha."
        )
    if not fail_trials.empty:
        fail_mats = ", ".join(fail_trials.drop_duplicates("Mat")["Mat"].tolist())
        st.error(
            f"**Avoid these maturity groups:** {fail_mats} — failed to reach harvest maturity."
        )
    ...
    ts_rec = ts[
        (ts["site"] == selected_site) & (ts["year"] == selected_year)
    ].sort_values("date")
    ...
    if macd_status == "declining" and heat_risk:
        days_to_heat = (
            int(
                (
                    fc_rec[fc_rec["maxt"] > 35]["date"].iloc[0] - fc_rec["date"].iloc[0]
                ).days
            )
            + 1
            if (fc_rec["maxt"] > 35).any()
            else 0
        )
        st.error(
            f"**Urgent: Heat stress incoming in {days_to_heat} days while vegetation is declining.** Consider irrigation and shade management."
        )
    elif macd_status == "declining" and rain_risk:
        st.warning(
            "**Monitor closely:** Vegetation declining and low rainfall forecast. Consider supplemental irrigation within 5 days."
        )
    elif macd_status == "declining":
        st.warning(
            "**Vegetation declining** (MACD negative). Monitor for continued stress and plan intervention."
        )
    elif cold_risk:
        days_to_cold = (
            int(
                (
                    fc_rec[fc_rec["mint"] < 0]["date"].iloc[0] - fc_rec["date"].iloc[0]
                ).days
            )
            + 1
        )
        st.warning(
            f"**Frost risk in {days_to_cold} days.** Consider protective measures for sensitive growth stages."
        )
    elif heat_risk:
        days_to_heat = (
            int(
        ...
        st.warning(
            f"**Heat stress expected in {days_to_heat} days.** Plan irrigation to mitigate impact."
        )
    elif macd_status == "improving":
        st.success(
            "**No action needed.** Vegetation is recovering and no extreme weather forecast."
        )
    else:
        st.success(
            "**No action needed.** Conditions are stable with no forecast risks."
        )
...
if not is_sce:
    ...
    if fc.empty:
        ...
        with st.expander("Full 16-day forecast chart"):
            ...
            fig_fc.add_trace(
                go.Scatter(
                    x=fc["date"], y=fc["maxt"], name="Max Temp", line=dict(color="red")
                ),
                secondary_y=False,
            )
            fig_fc.add_trace(
                go.Scatter(
                    x=fc["date"], y=fc["mint"], name="Min Temp", line=dict(color="blue")
                ),
                secondary_y=False,
            )
            fig_fc.add_trace(
        ...
        for idx, row in fc.iterrows():
            ...
            if row["maxt"] > 35:
                stress_days.append(
                    f"Day {day_num} ({row['date'].strftime('%b %d')}): heat stress likely (max {row['maxt']:.1f}°C)"
                )
            if row["mint"] < 0:
                stress_days.append(
                    f"Day {day_num} ({row['date'].strftime('%b %d')}): frost stress likely (min {row['mint']:.1f}°C)"
                )
        ...
        if not ts_sel.empty and "histogram" in ts_sel.columns:
            ...
            if len(hist_vals) >= 5:
                ...
                fig_proj.add_trace(
                    go.Scatter(
                        x=list(range(n_pts, n_pts + 16)),
                        y=projected.tolist(),
                        mode="lines+markers",
                        name="Projected (16 days)",
                        line=dict(color="red", dash="dash", width=2),
                        marker=dict(size=5),
                    )
                )
                fig_proj.add_hline(
                    y=0, line_dash="solid", line_color="black", line_width=1
                )
                ...
                if projected[-1] < 0 and slope < 0:
                    st.warning(
                        "MACD histogram trending negative — vegetation stress may intensify."
                    )
    ...
    stress_sel = stress[
        (stress["site"] == selected_site) & (stress["year"] == selected_year)
    ].copy()
    ...
    if stress_sel.empty:
        ...
        styled = (
            stress_sel[available]
            .style.apply(style_stress, axis=1)
            .format(
                {
                    "ndvi": "{:.4f}",
                    "macd": "{:.4f}",
                ...
                subset=[
                    c
                    for c in ["ndvi", "macd", "signal_line", "histogram", "residual"]
                    if c in available
                ],
            )
        )
    ...
    if not pheno_site.empty:
        ...
        st.dataframe(
            pheno_site[available_yoy].style.format(
                ...
                subset=[
                    c
                    for c in ["ndvi_mean", "ndvi_max", "ndvi_min"]
                    if c in available_yoy
                ],
            ),
...
if is_sce:
    ...
    yield_df = sce_trials.drop_duplicates(subset=["Mat"])[
        ["Mat", "Yield_Sim", "Result"]
    ].sort_values("Mat")
    color_map = {
        "Harvested at Maturity.": "#2ecc71",
        "Dead from Total Senescence.": "#e74c3c",
    }
    yield_df["color"] = yield_df["Result"].map(color_map).fillna("#95a5a6")
    fig_yield = go.Figure(
        go.Bar(
            x=yield_df["Mat"],
            y=yield_df["Yield_Sim"],
    ...
    fig_yield.update_layout(
        xaxis_title="Maturity Group",
        yaxis_title="Simulated Yield (kg/ha)",
        height=420,
        margin=dict(t=30, b=30),
    )
    ...
    if not iah4_ts.empty and not sce_first_daily.empty:
        ...
        fig_overlay.add_trace(
            go.Scatter(
                x=sce_first_daily["DOY"],
                y=sce_first_daily["FracGrowthRate"],
                mode="lines",
                name="SCE FracGrowthRate",
        ...
        st.caption(
            "Compares real satellite observations (IAH4, ~35 km away) with SCE simulation growth rate."
        )
    else:
    ...
    stage_colors = {
        "Germinating": "rgba(139,69,19,0.12)",
        "Emerging": "rgba(34,139,34,0.12)",
        "Vegetative": "rgba(50,205,50,0.12)",
        "EarlyFlowering": "rgba(255,215,0,0.15)",
        ...
        "Maturing": "rgba(160,82,45,0.15)",
        "Ripening": "rgba(139,69,19,0.15)",
    ...
    fig_growth.update_layout(
        yaxis_title="Cumulative Yield (kg/ha)",
        height=420,
        margin=dict(t=30, b=30),
    )
    ...
    fig_stress.update_yaxes(
        title_text="Water Stress", range=[0, 1.05], secondary_y=False
    )
    ...
    adv_df = sce_trials.drop_duplicates(subset=["Mat"])[
        ["Mat", "Yield_Sim", "DTM_Sim", "Result"]
    ].copy()
    adv_df["harvested"] = adv_df["Result"].str.contains(
        "Harvested", case=False, na=False
    )
    ...
    if not harv.empty:
        ...
        fig_adv.add_shape(
            type="rect",
            x0=best["DTM_Sim"].min() - 3,
            x1=best["DTM_Sim"].max() + 3,
            y0=best["Yield_Sim"].min() * 0.95,
            y1=best["Yield_Sim"].max() * 1.05,
            line=dict(color="green", width=2, dash="dash"),
            fillcolor="rgba(46,204,113,0.1)",
        )
        fig_adv.add_annotation(
            x=best["DTM_Sim"].mean(),
            y=best["Yield_Sim"].max() * 1.06,
            text="Optimal Zone",
            showarrow=False,
    ...
    if not harv.empty:
        ...
        st.success(
            f"**Best maturity group:** {best_mat} (yield: {best_yield:.0f} kg/ha)"
        )
        if not fail.empty:
...
st.markdown("---")
st.caption(
    "CDA 2026 Hackathon — Crop Stress Decision Support Dashboard | Data: Sentinel-2 NDVI, Open-Meteo Forecast, G2F Metadata, SCE Simulation"
)
